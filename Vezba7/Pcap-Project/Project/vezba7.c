...
int main()
{
	...
	char filter_exp[] = "ether src host b8:27:eb:d5:e1:cd and ip and (udp or tcp)";
	...
	// Chose one device from the list  		--------------1-----------------
    ...
    if ((device_handle = pcap_open_live( device->name,		// name of the device
                              ...
                              0,							// normal mode ----------------------2-----------------------
                              2000,							// read timeout, SET to 2 seconds ------------2----------------------
	...
	// Check the link layer. We support only Ethernet for simplicity. -------3---------
	...
	/* start the capture  max_packets == 5 */
...
void print_tcp_header(tcp_header * th)
{
	...
	printf("\n============================================================");
	printf("\n\tTRANSPORT LAYER  -  Transmision Control Protocol (TCP)");
	
	printf("\n\tSource port:\t\t%u", ntohs(th->src_port));
...
void packet_handler(unsigned char* param, const struct pcap_pkthdr* packet_header, const unsigned char* packet_data)
{
	...
	if (ih->next_protocol == 0x11)		//UDP
	{
		...
		uh_len = ntohs(uh->datagram_length);
		
		//App_length
		app_length = uh_len - sizeof(udp_header);
		
	}
	else if (ih->next_protocol == 0x06)		//TCP
	{
		th = (tcp_header*) ((unsigned char*)ih + ip_len);
		
		print_tcp_header(th);
		
		//App_data
		app_data = (unsigned char*) th + sizeof(tcp_header);		
		
		th_len = th->header_length*4;
		
		app_length = packet_header->len - th_len - ip_len - sizeof(ethernet_header);
	...
	/* ------------------11------------------ */
	 ...
	 print_raw_data(app_data, app_length);
