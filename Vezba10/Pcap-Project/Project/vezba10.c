...
struct timeval first_packet_t;
struct timeval last_packet_t;
...
int main()
{
	...
	printf("Select the type of traffic for statistics data: ");
	int mode;
...
void packet_handler(unsigned char* param, const struct pcap_pkthdr* packet_header, const unsigned char* packet_data)
{
	...
	if (counter == 1) 
	{
		first_packet_t.tv_sec = packet_header->ts.tv_sec;
		first_packet_t.tv_usec = packet_header->ts.tv_usec;
	}

	if (counter == 20)
	{
		last_packet_t.tv_sec = packet_header->ts.tv_sec;
		last_packet_t.tv_usec = packet_header->ts.tv_usec;

//		printf(" Time for 20 packets: %ld s",  last_packet_t.tv_sec - first_packet_t.tv_sec);
	}
