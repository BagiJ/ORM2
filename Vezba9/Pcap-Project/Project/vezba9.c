...
void packet_handler(unsigned char* user, struct pcap_pkthdr* packet_header, unsigned char* packet_data)
{
	...
	if (ntohs(eh->type) == 0x800)	// Ipv4
	{
		...
		if (ih->next_protocol == 0x06) // TCP
		{
			// change time_val before sending
			if (frame_counter == 10)
				packet_header->ts.tv_sec = 10;
			if (frame_counter == 11)
				packet_header->ts.tv_sec = 15;

			/**************************************************************/
			// Add packet in the queue
			if (pcap_sendqueue_queue(/*(pcap_sen_queue)*/queue_tcp, packet_header, packet_data) == -1)
			{
	...
	else if (ntohs(eh->type) == ETHERTYPE_ARP)
	{
		packet_data[6] = 0x2c;
		packet_data[7] = 0x4d;
		packet_data[8] = 0x54;
